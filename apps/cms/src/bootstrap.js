...
exports.default = async ({ strapi }) => {
    ...
    try {
        ...
        for (const contentType of publicReadableTypes) {
            const findPermission = permissions.find((p) => p.action === 'find' && p.subject === contentType);
            const findOnePermission = permissions.find((p) => p.action === 'findOne' && p.subject === contentType);
            // Enable find
            ...
            for (const action of actionsToDisable) {
                const permission = permissions.find((p) => p.action === action && p.subject === contentType);
        ...
        for (const contentType of privateTypes) {
            const typePermissions = permissions.filter((p) => p.subject === contentType);
